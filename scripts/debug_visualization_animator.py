...
class DebugVisualizer:
    ...
    def update_step(
        ...
        if isinstance(self.main_controller.fast_avoider, MixedEnvironmentAvoider):
            reference = (
                self.main_controller.fast_avoider.obstacle_avoider.reference_direction
            )
            ref_norm = LA.norm(reference)
            self.ax.arrow(
                global_ctrl_point[0],
            ...
            reference = (
                self.main_controller.fast_avoider.lidar_avoider.reference_direction
            )
            ref_norm = LA.norm(reference)
            self.ax.arrow(
                global_ctrl_point[0],
